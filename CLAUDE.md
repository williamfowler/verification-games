...
1. `eval_power_monitor.py` runs a sweep of transformer configs (split TRAIN/TEST), samples INA3221 power offline at 2 Hz, and fits the model by least squares (`E_net = a·TFLOPs + b·t_active → a=E_MARGINAL, b=POWER_OVERHEAD`) on TRAIN. It scores all runs against `FlopCounterMode` ground truth via `detect_flops.estimate_tflops` and reports per-workload error plus a `PASS/FAIL: held-out max err < 10%` verdict.
...
`eval_power_monitor.py` is the primary accuracy/calibration tool: it sweeps transformer configs (TRAIN/TEST split), scores `estimate_tflops` against ground truth, and prints a `RECOMMENDED CONSTANTS` block to paste into `detect_flops.py`. As of 2026-06-22 the shipped fit holds all 9 sweep configs to ≤9.3% error (held-out ≤6.4%).
